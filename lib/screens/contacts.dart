...
class ContactState extends State<ContactsPage> {
      ...
      return AlertDialog(  
        ...
        actions: <Widget>[  
          ...
          TextButton(  
            ...
            onPressed: () async{
              //Validation checks
              if(codeRole!=null && name!=null && number!=null && validatorName ==true && validatorNumber ==true){
  ...
  asyncDeleteDialog(BuildContext context, Contacts delAddress, int index) async {  
  return showDialog(  
    context: context,  
    barrierDismissible: false, // user must tap button for close dialog!  
    builder: (BuildContext context) {  
              ...
              children: [
                ...
                return Card(
                  child: ListTile(
                    ...
                    onLongPress: (){ 
                      setState(() async{
                      await asyncDeleteDialog(context, telBook[index], index);
                      });
                   },
                  ),
                );
...
startContacts() async{
    ...
    if(sp.getBool('initContacts')==null || sp.getBool('initContacts')==false){
...
updateBook() async{
  ...
  for(int i=0; i<nContacts; i++){
    if(sp.getString('Contact${i+1}')!=null && sp.getString('Contact$i')==null){
  ...
  for(int i=0; i<nContacts; i++){
    ...
    Contacts newAddress = Contacts.fromJson(await json.decode(sp.getString(nAddress)!));
